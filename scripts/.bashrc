## Sample RC file
## Should be placed into ~/.bashrc or ~/.profile
...
BAT_CHARGE=~/Repositories/Python/python-batterylevel/scripts/batterylevel.py
function battery_charge {
    echo `python $BAT_CHARGE` 2>/dev/null
}
...
function set_bash_prompt () {
  ...
  righthalf=`battery_charge`
  let fillsize=${COLUMNS}-${#lefthalf}-${#righthalf}+28     ## modify to get alignment corr +/-
  fill=`printf ' %.0s' {1..500}` # 300 spaces
  fill=${fill:0:$fillsize}

  # Set the bash prompt variable.
  PS1="\n$FGGR\u@$HOSTCOLOR\h$RESET $FGGR\w$RESET ${BRANCH}$fill$FGGR$(battery_charge) $RESET\n${PROMPT_SYMBOL} "
}
